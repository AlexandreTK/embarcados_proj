...
bool stop_alarm_sound =  false;
pthread_t current_thread = 0;
pthread_t last_thread    = 0;
pthread_t exited_thread  = 0;
//*******************************************
...
//************* SONG CODE ***************
struct play_song_args_t {
	char * song_path;
	pthread_t last_thread;
};

void * play_song(void * play_song_args) {
	
	last_thread = pthread_self();
	fprintf(stderr, "\n\nlast_thread: %d\n", last_thread);
	struct play_song_args_t * args = (struct play_song_args_t *) play_song_args;
	fprintf(stderr, "last_thread_args: %d\n", args->last_thread);
	
	if (args->last_thread != 0) {
		//while(pthread_kill(args->last_thread, 0) == 0) {
		//	sleep(1);
		//}
		while(exited_thread != args->last_thread) {
			fprintf(stderr, "exited_thread: %d\n", exited_thread);
			sleep(1);
		}
	}
	...
	char play_song_command[200];
	sprintf(play_song_command, "%s %s", MUSIC_PLAYER, args->song_path);
	//strcpy(play_song_command, MUSIC_PLAYER);
	//strcat(play_song_command, (char *) song_path);
	//fprintf(stderr, "%s\n", play_song_command);
	...
	if (fork_song_pid == 0) {
		...
		while(1) {
			if(stop_alarm_sound) {
				...
				fprintf(stderr, "Stop alarm received\n");
				kill(fork_song_pid, SIGTERM);
				fprintf(stderr, "killing process %d\n", fork_song_pid);
				sleep(1);
				stop_alarm_sound = false;
				fork_song_pid = 0;
				exited_thread = current_thread;
...
int main(int argc, char* argv[]) {
	...
	while(1) {
		...
		if (play_song_secs >= 0) {
			stop_alarm_sound = false;
			struct play_song_args_t play_song_args;
			play_song_args.last_thread = last_thread;	
			play_song_args.song_path = song_to_play_path;
			if (pthread_create(&thread_motor, NULL, play_song, (void *) &play_song_args)) {
				fprintf(stderr, "An error occured while creating new thread");
				return 1;
			}
		...
		*/
		...
		if (play_song_secs==-10) {
			stop_alarm_sound = false;
			struct play_song_args_t play_song_args;
			play_song_args.last_thread = last_thread;	
			play_song_args.song_path = REMINDER_PATH;
			if (pthread_create(&thread_motor, NULL, play_song, (void *)&play_song_args)) {
				fprintf(stderr, "An error occured while creating new thread");
				return 1;
			}
